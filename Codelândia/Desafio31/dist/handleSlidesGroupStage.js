...
let dataRoundInMatch;
let control = 0;
export function roundInMacthes(data) {
    dataRoundInMatch = data.filter((match) => match.round !== "");
}
function createRounds(element) {
    element.innerHTML +=
        `
  ...
  `;
    ...
    if (controllers instanceof NodeList)
        controllers.forEach((item) => item.addEventListener("click", (event) => {
...
export function insertRounds() {
    const contentGroups = document.querySelector('.contentGroups');
    const groups = document.querySelector('[data-stage]');
    const classGroups = document.querySelector('[data-stage]')?.classList.contains('stages');
...
function roundOne() {
    const title = document.querySelector('[data-title-round]');
    const contentRound = document.querySelector('[data-round]');
...
function controlSlideRound(event, controllers) {
...
//# sourceMappingURL=handleSlidesGroupStage.js.map